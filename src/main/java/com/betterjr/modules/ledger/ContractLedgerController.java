...
public class ContractLedgerController {
    ...
    public @ResponseBody String queryContractLedger(HttpServletRequest request, String queryType, int pageNum, int pageSize) {
        ...
        return exec(() -> contractLedgerService.webQueryContractLedgerByPage(map, queryType, pageNum, pageSize), "分页查询贸易合同台账", logger);
    }
    ...
    public @ResponseBody String findContractLedgerCustInfoByCustNoAndContractId(Long custNo,Long contractId) {
        logger.info("findContractLedgerCustInfoByCustNoAndContractId，入参，custNo:"+ custNo+"，contractid:"+contractId);
        return exec(() -> contractLedgerService.webFindCustInfoByCustNoAndContractId(custNo, contractId), "查询合同台账客户信息", logger);
    }
