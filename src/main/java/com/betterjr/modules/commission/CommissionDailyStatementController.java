...
public class CommissionDailyStatementController {
    ...
    public @ResponseBody String findDailyStatementInfoByMonth(String billMonth, Long custNo, String businStatus) {
        ...
        return exec(() -> dailyStatementService.webFindDailyStatementInfoByMonth(billMonth, custNo, businStatus),
                "根据对账月份查询日账单列表", logger);
    }
    ...
    public @ResponseBody String updateDailyStatement(Long dailyStatementId, String businStatus) {
        ...
        return exec(() -> dailyStatementService.webUpdateDailyStatement(dailyStatementId, businStatus), "更新日报表状态",
                logger);
    }
    ...
    public @ResponseBody String queryPayResultRecord(Long custNo, String payDate, String payStatus, int flag,
            int pageNum, int pageSize) {
        String anPayDate = payDate.replace("-", "");
        logger.info("分页查询支付结果,入参：custNo:" + custNo + ",payDate:" + anPayDate + "，payStatus：" + payStatus);
        return exec(() -> dailyStatementService.webQueryPayResultRecord(custNo, anPayDate, payStatus, flag, pageNum,
                pageSize), "分页查询支付结果", logger);
    }
    ...
    public @ResponseBody String queryDailyStatementRecordByDailyId(Long dailyStatementId, String flag, int pageNum,
            int pageSize) {
        logger.info("分页查询日账单记录列表,入参dailyStatementId:" + dailyStatementId);
        return exec(
                () -> dailyStatementService.webQueryDailyStatementRecordById(dailyStatementId, pageNum, pageSize, flag),
                "查询日账单记录列表", logger);
    }
    ...
    public @ResponseBody String findDailyStatementByPayDate(String payDate, Long custNo) {
        ...
        return exec(() -> dailyStatementService.webFindDailyStatementByPayDate(payDate, custNo), "根据对账日期和对账企业查询日账单",
                logger);
    }
}
